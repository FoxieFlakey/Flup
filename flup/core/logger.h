...
/**
 * @brief Structure about @ref printk call site
 *
 * Optionally by default used to track where @ref printk
 * was called from to aid in more detailed log.
 */
typedef struct printk_call_site_info {
  /**
   * @brief Pointer to function which called @ref printk
   * 
   ...
   * to @ref printk. The function will try fill this if
...
/// @{
/**
 * @brief Short wrapper around @ref printk with coresponding level
 */
#define pr_fatal(fmt, ...) printk(FLUP_FATAL, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_alert(fmt, ...) printk(FLUP_ALERT, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_critical(fmt, ...) printk(FLUP_CRITICAL, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_error(fmt, ...) printk(FLUP_ERROR, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_warn(fmt, ...) printk(FLUP_WARN, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_notice(fmt, ...) printk(FLUP_NOTICE, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_info(fmt, ...) printk(FLUP_INFO, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_verbose(fmt, ...) printk(FLUP_VERBOSE, fmt __VA_OPT__(,) __VA_ARGS__)
#define pr_debug(fmt, ...) printk(FLUP_DEBUG, fmt __VA_OPT__(,) __VA_ARGS__)
/// @}
